...
Spec reference: ADR-2026-0007 (tool-use loop paralelo), SDD §4 (Ciclo PRAL — fase Reasoning)
"""
...
from app.llm_metrics import LLM_CALL_DURATION, LLM_CALLS_TOTAL
    ...
    async def analyze(self) -> SpecialistFinding:
        ...
        while True:
            if iterations >= MAX_TOOL_ITERATIONS:
                ...
                LLM_CALLS_TOTAL.labels(call_type="specialist", outcome="error").inc()
                LLM_CALL_DURATION.labels(call_type="specialist").observe(time.monotonic() - _start)
            ...
            try:
                ...
                LLM_CALLS_TOTAL.labels(call_type="specialist", outcome="circuit_open").inc()
                LLM_CALL_DURATION.labels(call_type="specialist").observe(time.monotonic() - _start)
                return SpecialistFinding(
                    specialist=self.name,
                    severity=Severity.warning,
            ...
            if response.stop_reason == "end_turn":
                LLM_CALLS_TOTAL.labels(call_type="specialist", outcome="success").inc()
                LLM_CALL_DURATION.labels(call_type="specialist").observe(time.monotonic() - _start)
